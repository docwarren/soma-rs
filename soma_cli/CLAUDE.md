...
- `src/main.rs` — CLI entry point. Routes commands to `soma_core::api` search functions.
- `src/jam/` — JAM format module with its own models and constants.

## Key Patterns

- Detects file type from extension via `soma_core::utils` and dispatches to the appropriate search function.
- Outputs results in the format determined by `OutputFormat` (JSON, TSV, etc.).
- All genomic parsing is delegated to `soma_core`.
